...
async def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        pdf_pages = results.get_text_documents(split_by_page=True)
        print(f"Successfully extracted {len(pdf_pages)} pages from PDF")
...
async def process_pdf_table(pdf_path, page_images, output_dir="./extracted_tables"):
    ...
    try:
        ...
        pdf_pages = await extract_text_from_pdf(pdf_path)
        if not pdf_pages:
            print("Failed to extract text from PDF. Aborting process.")
            return False
...
async def main(image_folder_path, output_dir="./extracted_tables"):
    ...
    # Step 2: Convert PDF pages to individual images
    page_image_paths = await convert_pdf_to_images(pdf_path)
    if not page_image_paths:
        print("Failed to convert PDF pages to images. Aborting process.")
        return
    
    print(f"Converted PDF to {len(page_image_paths)} page images")
    
    # Step 3: Process PDF with LlamaParse and extract tables
    classifier = await process_pdf_table(pdf_path, page_image_paths, output_dir)
    ...
    if classifier:
        ...
        summary = {
            "pdf_path": pdf_path,
            "number_of_pages": len(page_image_paths),
